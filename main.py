...
# ============================================================
# Upload status DB (READ / WRITE)
db_session = create_upload_session()  # upload_status
fr_session = create_factory_session()  # factory_reports
...
if not not_uploaded_wafermaps:
    ...
    print(
        f"\nProcessing {len(zip_to_process)} ZIPs containing "
        f"{len(not_uploaded_wafermaps)} NOT_UPLOADED wafermaps..."
    )
            ...
            with zipfile.ZipFile(zip_path, "r") as zf:
                ...
                for root, _, files in os.walk(extract_dir):
                    ...
                    if upload_and_verify(
                        c, d, umc_file, FTP_BASE_URL, TEMP_DL_DIR, MAX_FTP_RETRIES
                    ):
                        uploaded_wafers += 1

                        # -------------------------
                        # Step 9: DB UPSERT (only after FTP OK)
                        # -------------------------
